...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Only load video if URL is provided
    if (videoUrl && videoUrl !== "/video.mp4") {
      video.src = videoUrl
      video.load()
    } else {
      // No video URL provided, show message
      setHasError(true)
      setIsLoading(false)
    }
  ...
  return (
    <div className="fixed inset-0 w-screen h-screen flex items-center justify-center bg-black">
      <div className="relative w-full h-full overflow-hidden">
        ...
        {hasError && (
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-white text-lg text-center">
            <div>Animation unavailable</div>
            <div className="text-sm mt-2 opacity-75">
              Add NEXT_PUBLIC_VIDEO_URL environment variable
            </div>
          </div>
        )}
      </div>
